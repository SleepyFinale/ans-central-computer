...
if ! echo "$PKG_LIST_NAV2" | grep -q "^nav2_msgs$"; then
    ...
    echo "    ./scripts/build/clean_rebuild.sh"
    echo ""
    exit 1
else
...
# User can override with: PARALLEL_JOBS=N ./scripts/build/clean_rebuild.sh
...
echo "  - To override: PARALLEL_JOBS=N ./scripts/build/clean_rebuild.sh"
echo ""
