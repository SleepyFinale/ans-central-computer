...
#  - ./scripts/core/start_central.sh        (TF relay, map merge, multi_robot_explorer)
#  - ./scripts/core/start_rviz_central.sh   (RViz configs)
...
if ! echo "$PKG_LIST_NAV2" | grep -q "^nav2_msgs$"; then
    ...
    echo "    ./scripts/build/minimal_rebuild.sh"
    echo ""
    exit 1
else
...
# Build only required packages for the central workflow
echo ""
echo "Step 5: Building minimal central package set..."
echo "  Building packages needed for:"
echo "    - ./scripts/core/start_central.sh (TF relay, map merge, explorer)"
echo "    - ./scripts/core/start_rviz_central.sh (RViz configs)"
echo ""
...
# User can override with: PARALLEL_JOBS=N ./scripts/build/minimal_rebuild.sh
...
echo "  - To override: PARALLEL_JOBS=N ./scripts/build/minimal_rebuild.sh"
echo ""

colcon build --symlink-install --parallel-workers $PARALLEL_JOBS \
    --cmake-args -DBUILD_TESTING=OFF \
    --packages-up-to multirobot_map_merge
