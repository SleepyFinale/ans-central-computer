...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "Important: This script ran in a subshell. In each terminal where you"
    echo "run ROS 2 commands (e.g. ros2 topic list, SLAM, Nav2, explorer), run:"
    echo ""
    echo "  cd ~/turtlebot3_ws"
    echo "  source install/setup.bash"
    echo "  source scripts/set_robot_env.sh <robot>   # e.g. blinky, pinky, inky <IP>, clyde <IP>"
    echo ""
    echo "Without sourcing the workspace and setting the robot environment"
    echo "(ROS_DOMAIN_ID), you will not see topics from the robot."
    echo ""
else
    echo ""
    echo "=========================================="
