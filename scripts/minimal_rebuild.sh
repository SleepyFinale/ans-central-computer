...
if [ $? -eq 0 ]; then
    ...
    echo "run ROS 2 commands, run: source install/setup.bash and"
    echo "source scripts/set_robot_env.sh <robot> (e.g. blinky) so you see"
    echo "the robot's topics (ROS_DOMAIN_ID must match the robot)."
    echo ""
    ...
    echo "  4. ./scripts/start_explorer_simple.sh"
else
    echo ""
    echo "=========================================="
    echo "✗ Build failed!"
