...
std::vector < nav_msgs::msg::OccupancyGrid::ConstSharedPtr > loadMaps(
  InputIt filenames_begin,
  ...
  const nav_msgs::msg::OccupancyGrid::ConstSharedPtr & map)
{
  ...
  cv::Mat img(
    map->info.height, map->info.width, CV_8S,
...
// ignores header, map_load_time and origin
static inline bool maps_equal(
  const nav_msgs::msg::OccupancyGrid & grid1,
