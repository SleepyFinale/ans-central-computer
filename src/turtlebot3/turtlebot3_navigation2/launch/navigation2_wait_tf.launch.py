...
#
# Modified to wait for TF tree before starting Nav2
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # Wait for TF tree (if enabled) - this will block until TF is ready
        ExecuteProcess(
            cmd=['python3', wait_tf_script],
            output='screen',
            condition=lambda context: context.launch_configurations.get('wait_for_tf', 'true') == 'true'
        ),

        # Start Nav2 (will start after TF wait completes)
        ...
        # Start RViz (can start immediately)
        Node(
            package='rviz2',
            executable='rviz2',
            name='rviz2',
